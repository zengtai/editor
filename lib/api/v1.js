...
import { RENEW_DATA_FROM_REMOTE as RENEW_DATA } from "../constants";
...
export const dataFormatter = async (data) => {
  ...
  original.forEach((game) => {
    ...
    games.sort((a, b) => (a.creation_date < b.creation_date ? 1 : -1));
    // 写入分类数组
...
export const localCache = async (type) => {
  ...
  // const original = path.join(process.cwd(), `/data/original.db`); // 源数据
  // const games = path.join(process.cwd(), `/data/games.db`); // 处理后的游戏数据
  // const categories = path.join(process.cwd(), `/data/categories.db`); // 分类数据
  // const search = path.join(process.cwd(), `/data/search.db`); // 搜索结果数据
  ...
  if (RENEW_DATA || !!filesExist) {
    ...
    if (type) {
      ...
      localDataInfo.forEach((i) => {
        let tmp = fs.readFileSync(i.path, (e) => {
          console.error(e);
        });
        tmpData[i.name] = JSON.parse(tmp);
      });
