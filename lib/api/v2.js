export const CMS_API_V2 = `https://cms.uptap.com`; // POST
...
export const getDataForHome = async (limit = 10) => {
  const data = await fetchAPI(
    `
    query getGamesForHome ($where: Int) {
      Games (sort: "-creation_date", limit: $where, filter: { status: { _eq: "published" } }) {
        title
        slug
        icon_url
        appid
...
export const getAllGamesWithSlug = async () => {
  const data = await fetchAPI(
    ...
    query getAllGamesWithSlug  {
      games (sort: "-creation_date", filter: { status: { _eq: "published" } }) {
        slug
      }
    }
  `
...
export const getGamesByCategory = async (category) => {
  const data = await fetchAPI(
    ...
    query getGamesByCategorySlug ($where: String)  {
      games: Games (filter: { category: { name: { _eq: $where } } }) {
        title
        category {
          name
          slug
...
export const getGamesBySlug = async (slug, limit = 10) => {
  const data = await fetchAPI(
    `
